...
/// Used to not block the UI isolate.
Future<StreamManifest> computeStreamManifest(String videoId) async {
  YoutubeExplode yt = YoutubeExplode();

  StreamManifest manifest = await yt.videos.streamsClient.getManifest(videoId);
  manifest.streams.map((e) => e.toJson());

  return manifest;
}

/// Used to not block the UI isolate.
Future<String> computeSubtitle(ClosedCaptionTrackInfo info) async {
  YoutubeExplode yt = YoutubeExplode();
  String vtt = await yt.videos.closedCaptions.getSubTitles(info);
  ...
  return vtt.replaceAllMapped(regExp, (match) {
    String text = match.group(11) ?? '';
    List<String> lines = text.split('\n');

    String currentSubtitle = vtt.substring(match.start, match.end);
    int position =
...
class PlayerYoutubeSource extends PlayerMediaSource {
  ...
  Future<StreamManifest> getStreamManifest(MediaItem item) async {
    ...
    if (manifest == null) {
      manifest = await compute(computeStreamManifest, videoId);
  ...
  Future<List<SubtitleItem>> prepareSubtitles({
    ...
    for (ClosedCaptionTrackInfo trackInfo in tracks) {
      ...
      String vtt = await compute(computeSubtitle, trackInfo);
        ...
        SubtitleItem(
          controller: SubtitleController(
            provider: SubtitleProvider.fromString(
              ...
              startOffset: trackInfo.isAutoGenerated
                  ? const Duration(milliseconds: 0)
                  : Duration.zero,
              endOffset: trackInfo.isAutoGenerated
                  ? const Duration(milliseconds: 0)
                  : Duration.zero,
            ),
          ),
  ...
  String getVideoUrlForQuality({
    ...
    for (VideoStreamInfo streamInfo in manifest.video.sortByBitrate()) {
  ...
  Future<String> getAudioUrl(MediaItem item) async {
    ...
    AudioStreamInfo streamAudioInfo =
        manifest.audio.sortByBitrate().firstWhere((info) {
