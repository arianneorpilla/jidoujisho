...
class PlayerYoutubeSource extends PlayerMediaSource {
  ...
  /// Auto generated subtitles need to have their first line stripped. Returns
  /// text for a VTT file.
  String stripFirstLine(String vttContent) {
    ...
    return vttContent.replaceAllMapped(regExp, (match) {
      String text = match.group(11) ?? '';
      List<String> lines = text.split('\n');

      String currentSubtitle = vttContent.substring(match.start, match.end);
      int position =
          currentSubtitle.lastIndexOf(text).clamp(0, currentSubtitle.length);

      String newSubtitle =
          currentSubtitle.replaceFirst(text, lines.last.trim(), position);
