...
const double _kToolbarHeight = 300;
...
class JidoujishoTextSelectionToolbar extends StatelessWidget {
  ...
  const JidoujishoTextSelectionToolbar({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final Offset anchorAbovePadded =
        anchorAbove - const Offset(0, _kToolbarContentDistance);
    final Offset anchorBelowPadded =
        anchorBelow + const Offset(0, _kToolbarContentDistanceBelow);
...
class _RenderTextSelectionToolbarItemsLayout extends RenderBox
  ...
  void _placeChildren() {
    ...
    visitChildren((RenderObject renderObjectChild) {
      ...
      if (!overflowOpen) {
        childParentData.offset = Offset(fitWidth, 0);
        ...
        childParentData.offset = Offset(0, overflowHeight);
    ...
    if (_shouldPaintChild(firstChild!, 0)) {
      ...
      if (overflowOpen) {
        navButtonParentData.offset =
            isAbove ? Offset(0, overflowHeight) : Offset.zero;
        nextSize = Size(
          nextSize.width,
          isAbove ? nextSize.height + navButton.size.height : nextSize.height,
        );
      } else {
        navButtonParentData.offset = Offset(fitWidth, 0);
