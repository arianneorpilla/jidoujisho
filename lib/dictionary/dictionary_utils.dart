...
Future<void> dictionaryFileImport(
    BuildContext context, DictionaryFormat dictionaryFormat) async {
  /// Used for database operations and collision checks.
  ...
  try {
    ...
    workingDirectory = await compute(
      dictionaryFormat.prepareWorkingDirectory,
    ...
    List<DictionaryEntry> dictionaryEntries = await compute(
      dictionaryFormat.extractDictionaryEntries,
      importProcessingParams,
    );
...
void importMessagePrepare(ValueNotifier<String> progressNotifier) {
  progressNotifier.value = "Preparing working space...";
}
