...
Future<YouTubeMux> getPlayerYouTubeInfo(String webURL) async {
  ...
  if (videoID != null) {
    ...
    for (var stream in streamManifest.video.sortByBitrate()) {
...
Future<void> initialiseYouTubeDL() {
  YoutubeDL.init(null);
}

Future<String> requestAutoGeneratedSubtitles(String url) async {
  File autogenSubFile = File('$appDirPath/jidoujisho-autogenSubtitles');
  if (autogenSubFile.existsSync()) {
    autogenSubFile.deleteSync();
  }
  ...
  File vttFile = (Directory("$appDirPath").listSync().firstWhere((fileEntity) =>
      path
          .basename(fileEntity.path)
          .startsWith("jidoujisho-autogenSubtitles"))) as File;
